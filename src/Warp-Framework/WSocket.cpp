	...
	, m_bufferSize(bufferSize)
{
	...
	m_AcceptReg = new WRegistry();
	m_BindReg = new WRegistry();
	m_ConnectReg = new WRegistry();
	m_ListenReg = new WRegistry();
	m_ReceiveReg = new WRegistry();
	m_SendReg = new WRegistry();
}
...
W_INT WSSocket::Accept(int& outClientSize)
{
	...
	std::unique_ptr<WSocketArgs> args = std::make_unique<WSocketArgs>(WSocketArgs(newConnection, nullptr, 0));
...
W_INT WSSocket::Bind(void)
{
	...
	std::unique_ptr<WSocketArgs> args = std::make_unique<WSocketArgs>(WSocketArgs(m_sock, nullptr, 0));
...
W_INT WSSocket::Receive(char* outData, int& lengthTransfer)
{
	...
	std::unique_ptr<WSocketArgs> args = std::make_unique<WSocketArgs>(WSocketArgs(m_sock, m_recvBuffer, bytesReceived));
...
W_INT WSSocket::Send(char* inData, int& lengthTransfer)
{
	...
	std::unique_ptr<WSocketArgs> args = std::make_unique<WSocketArgs>(WSocketArgs(m_sock, inData, lengthTransfer));
