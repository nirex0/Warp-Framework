...
void WLerp::WorkerWork(void)
{
	...
	while (!isNear(m_value, m_to, 1))
	{
		...
		std::unique_ptr<WLerpArgs> lerpArgsTick = std::make_unique<WLerpArgs>(WLerpArgs(m_value));
		m_LerpTickRegistry->Run(this, lerpArgsTick.get());
	}
	m_isRunning = false;
	std::unique_ptr<WLerpArgs> lerpArgsDone = std::make_unique<WLerpArgs>(WLerpArgs(m_to));
