...
void WColorTransform::WorkerWork(void)
{
		...
		!isNear(m_value.a, m_to.a, 0.01F))
	{
		...
		std::unique_ptr<WColorTransformArgs> CTArgsTick = std::make_unique<WColorTransformArgs>(WColorTransformArgs(m_value));
		m_CTTickRegistry->Run(this, CTArgsTick.get());
	}
	m_isRunning = false;
	std::unique_ptr<WColorTransformArgs> CTArgsDone = std::make_unique<WColorTransformArgs>(WColorTransformArgs(m_to));
